...
export const generateMockSPU = (overrides?: Partial<SPUItem>): SPUItem => {
  ...
  const categoryLevel3 = categoryLevel2
    ? randomChoice(cinemaCategories.level3[categoryLevel2] || [])
    : undefined;
  ...
  return {
    ...
    categoryName: categoryLevel3
      ? `${categoryName} > ${categoryLevel2} > ${categoryLevel3}`
    ...
    tags: [
      randomChoice(['热销', '新品', '推荐', '限时优惠']),
      randomChoice(['经典', '网红', '健康']),
    ],
...
export const generateMockCategory = (level: CategoryLevel, parentId?: string): CategoryItem => {
  ...
  if (level === 1) {
    ...
    if (parentCategory) {
      const level2Options =
        cinemaCategories.level2[parentCategory as keyof typeof cinemaCategories.level2] || [];
