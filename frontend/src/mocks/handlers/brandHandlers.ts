...
export const createBrandHandler = http.post('/api/v1/brands', async ({ request }) => {
  ...
  const existingBrand = mockBrands.find(
    (b) => b.name === brandData.name && b.brandType === brandData.brandType
  );
...
export const updateBrandHandler = http.put('/api/v1/brands/:id', async ({ params, request }) => {
  ...
  // 检查品牌名称重复（排除自己）
  const existingBrand = mockBrands.find(
    (b) => b.id !== id && b.name === updateData.name && b.brandType === updateData.brandType
  );
...
// 更改品牌状态API
export const updateBrandStatusHandler = http.patch(
  '/api/v1/brands/:id/status',
...
// 上传品牌LOGO API
export const uploadBrandLogoHandler = http.post(
...
// 获取品牌使用统计API
export const getBrandUsageStatsHandler = http.get(
  '/api/v1/brands/:id/usage-stats',
  async ({ params }) => {
    const { id } = params;
    const brand = mockBrands.find((b) => b.id === id);
