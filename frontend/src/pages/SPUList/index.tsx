...
const SPUListPage: React.FC<SPUListProps> = () => {
  ...
  const handleExport = useCallback(() => {
    spuService.batchExportSPU(selectedRowKeys.length > 0 ? selectedRowKeys as string[] : undefined)
      .then(response => {
        if (response.success) {
          // 模拟下载
  ...
  // 处理批量状态更新
  const handleBatchStatusChange = useCallback((ids: string[], status: SPUStatus) => {
    spuService.batchUpdateSPUStatus(ids, status)
      .then(response => {
        if (response.success) {
          message.success(response.message)
          setSelectedRowKeys([])
  ...
  // 处理单行操作
  const handleView = useCallback((record: SPUItem) => {
    navigate(`/spu/${record.id}`)
  }, [navigate])

  const handleEdit = useCallback((record: SPUItem) => {
    navigate(`/spu/${record.id}/edit`)
  }, [navigate])

  const handleDelete = useCallback((record: SPUItem) => {
    spuService.deleteSPU(record.id)
      .then(response => {
        if (response.success) {
  ...
  const handleCopy = useCallback((record: SPUItem) => {
    spuService.batchCopySPU([record.id])
      .then(response => {
        if (response.success) {
          message.success(response.message)
          // 重新加载数据
