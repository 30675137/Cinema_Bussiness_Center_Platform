  ...
  const testSearchFunctionality = async () => {
    ...
    try {
      ...
      if (searchResponse.success) {
        ...
        // 验证搜索结果是否正确
        const allMatchKeyword = searchResponse.data.list.every(item =>
  ...
  const testPaginationFunctionality = async () => {
    ...
    try {
      ...
      if (page1Response.success && page2Response.success) {
        ...
        addTestResult(hasOverlap ? '❌ 分页数据验证失败，存在重复数据' : '✅ 分页数据验证通过，无重复数据')
      }
    } catch (error) {
