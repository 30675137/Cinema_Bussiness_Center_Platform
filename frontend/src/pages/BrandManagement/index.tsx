...
const BrandManagementPage: React.FC<BrandManagementProps> = () => {
  ...
  const handleBatchOperation = (operation: 'activate' | 'deactivate' | 'delete') => {
    message.info(`批量${operation === 'activate' ? '启用' : operation === 'deactivate' ? '停用' : '删除'}功能开发中...`)
  }
                  ...
                  <Card title="信息完整性统计" size="small">
                    <div style={{ padding: '24px 0' }}>
                      <div style={{ marginBottom: 16 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <Text>有官网的品牌</Text>
                          <Text strong style={{ fontSize: '18px' }}>{stats.withWebsite}</Text>
                        </div>
                      </div>
                      <div style={{ marginBottom: 16 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <Text>有联系人的品牌</Text>
                          <Text strong style={{ fontSize: '18px' }}>{stats.withContact}</Text>
                        </div>
                      </div>
                      <div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          ...
                          <Text strong style={{ fontSize: '18px' }}>
                            {stats.total > 0 ? Math.round(((stats.withWebsite + stats.withContact) / (stats.total * 2)) * 100) : 0}%
                          </Text>
                        </div>
                      </div>
                    </div>
