...
import type { Brand, BrandFilters, BrandPaginationResponse } from '../types/brand.types';
...
export const useBrandList = (options: UseBrandListOptions = {}) => {
  ...
  const statusChangeMutation = useMutation({
    mutationFn: ({ id, status, reason }: { id: string; status: BrandStatus; reason?: string }) =>
  ...
  const changeBrandStatus = useCallback(
    async (brandId: string, newStatus: BrandStatus, reason?: string) => {
