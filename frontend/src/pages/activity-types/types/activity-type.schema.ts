...
export const createActivityTypeSchema = z.object({
  name: z.string().min(1, '活动类型名称不能为空').max(100, '活动类型名称长度不能超过100个字符'),
  description: z.string().max(500, '描述长度不能超过500个字符').nullable().optional(),
  businessCategory: z.string().max(100, '业务分类长度不能超过100个字符').nullable().optional(),
  backgroundImageUrl: z
    ...
    .nullable()
    .optional(),
  sort: z.number().int('排序号必须是整数').min(0, '排序号不能小于0'),
});
