...
export const SKUSchema = z.object({
  ...
  name: z.string().min(1, 'SKU名称不能为空').max(100, 'SKU名称不超过100字符'),

  price: z.number().int('价格必须为整数').nonnegative('价格不能为负数'),

  stockQuantity: z.number().int('库存数量必须为整数').nonnegative('库存数量不能为负数'),
...
export const SPUSchema = z.object({
  ...
  name: z.string().min(1, 'SPU名称不能为空').max(200, 'SPU名称不超过200字符'),

  categoryId: z.string().min(1, '产品分类不存在'),

  brandId: z.string().nullable(),

  description: z.string().max(2000, '描述不超过2000字符'),
...
export const BOMComponentSchema = z.object({
  ...
  standardCost: z.number().nonnegative('标准成本不能为负数').nullable(),

  status: BOMComponentStatusSchema,

  sortOrder: z.number().int('排序顺序必须为整数').nonnegative('排序顺序不能为负数'),
...
export const BOMSchema = z.object({
  ...
  name: z.string().max(100, '配方名称不超过100字符').nullable(),

  wasteRate: z.number().min(0, '损耗率必须在 0-100 之间').max(100, '损耗率必须在 0-100 之间'),

  status: BOMStatusSchema,

  components: z.array(BOMComponentSchema).min(1, 'BOM必须至少包含一个原料'),
 ...
 */
export const SKUUpdateRequestSchema = z
  .object({
    code: z
      .string()
      .min(3, 'SKU编码至少3个字符')
    ...
    name: z.string().min(1, 'SKU名称不能为空').max(100, 'SKU名称不超过100字符').optional(),

    price: z.number().int('价格必须为整数').nonnegative('价格不能为负数').optional(),
