...
vi.mock('../CategorySelector', () => ({
  CategorySelector: ({ placeholder }: any) => (
    <div data-testid="category-select">{placeholder}</div>
  ),
}));
...
const mockBrands = [{ id: '1', name: 'Test Brand', code: 'TB' }];

const mockCategories = [{ id: '1', name: 'Test Category', code: 'TC' }];
...
describe('SPUForm Attribute Template Integration', () => {
  ...
  it('应该在未选择模板时显示提示信息', () => {
    ...
    expect(
      screen.getByText('请在上方"属性模板"字段中选择一个模板以启用结构化属性管理')
    ).toBeInTheDocument();
  });
