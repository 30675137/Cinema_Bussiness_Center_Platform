...
import { useCategoryTreeQuery, useCategoryChildrenQuery, useCategorySearchQuery } from '@/hooks/api/useCategoryQuery'
...
const CategoryTree: React.FC<CategoryTreeProps> = ({
  ...
  // 构建树形数据（支持搜索高亮）
  const buildTreeData = useCallback((data: CategoryTreeType[] | undefined, searchResults?: Category[]): DataNode[] => {
    if (!data) return []
  ...
  // 渲染树节点标题
  const renderTreeNodeTitle = useCallback((category: CategoryTreeType, isHighlighted: boolean = false) => {
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1 }}>
          ...
          {category.status === 'active' && (
            <Tag color="green" style={{ fontSize: '11px' }}>启用</Tag>
          )}
        </div>
  ...
  }, [selectedCategoryId, showActions, mode, onCategorySelect, onCategoryEdit, onCategoryAdd, setSelectedCategoryId])
  ...
  const stats = useMemo(() => {
    ...
    const countNodes = (nodes: CategoryTreeType[]): { total: number; active: number; level1: number; level2: number; level3: number } => {
      let total = 0
      let active = 0
      let level1 = 0
      let level2 = 0
      ...
      <div style={{ 
        ...
        {isLoading && finalTreeData.length === 0 ? (
          ...
          <div style={{
            ...
            <div>
              {searchKeyword ? '没有找到匹配的类目' : '暂无类目数据'}
            </div>
