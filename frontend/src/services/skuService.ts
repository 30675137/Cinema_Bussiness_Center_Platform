...
export interface ISkuService {
   ...
   * 更新BOM配方
   */
  updateBom(skuId: string, components: Array<{
    componentId: string;
    quantity: number;
    unit: string;
    isOptional?: boolean;
 ...
 */
function getSpuInfoById(spuId: string, spuMap?: Map<string, BackendSpu>): { name: string; brand: string; category: string; spec: string } | null {
  // 从缓存中查找
  const cachedSpu = spuMap?.get(spuId);
  if (cachedSpu) {
    // 将规格列表拼接为字符串
    const specStr = cachedSpu.specifications
 ...
 * 将后端SKU数据转换为前端格式
 */
function transformBackendSku(backendSku: BackendSkuData, spuMap?: Map<string, BackendSpu>): SKU & { bomComponents?: any[] } {
...
class SkuServiceImpl implements ISkuService {
  ...
  async getSkuById(id: string): Promise<SKU> {
    ...
    const response = await apiService.get<{ success: boolean; data: BackendSkuData }>(`/skus/${id}`);
  ...
  async updateSku(id: string, formData: SkuFormData): Promise<SKU> {
    ...
    const response = await apiService.put<{ success: boolean; data: BackendSkuData }>(`/skus/${id}`, {
      name: formData.name,
      mainUnit: formData.mainUnitId,
      status: statusValue,
   ...
   * @param wasteRate 损耗率
   */
  async updateBom(skuId: string, components: Array<{
    componentId: string;
    quantity: number;
    unit: string;
    isOptional?: boolean;
