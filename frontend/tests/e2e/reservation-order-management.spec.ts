...
test.describe('Reservation Order Management', () => {
  ...
  test.describe('Reservation Order List Page', () => {
    test.beforeEach(async ({ page }) => {
      ...
      await page.waitForSelector('.ant-table, [data-testid="reservation-order-list"]', { timeout: 10000 }).catch(async () => {
    ...
    test('should support search by order number', async ({ page }) => {
      // Find search input
      const searchInput = page.locator('input[placeholder*="搜索"], input[placeholder*="预约单号"]').first();
    ...
    test('should navigate to detail page when clicking view button', async ({ page }) => {
      // Wait for table rows
      await page.waitForSelector('.ant-table-tbody tr, tbody tr', { timeout: 5000 }).catch(() => {});
      
      // Find view/detail button
      const viewButton = page.locator('button:has-text("查看"), button:has-text("详情"), a:has-text("详情")').first();
  ...
  test.describe('Reservation Order Detail Page', () => {
    test('should display order detail information', async ({ page }) => {
      ...
      if (await firstRow.count() > 0) {
        ...
        const detailCard = page.locator('.ant-card, .ant-descriptions, [data-testid="order-detail"]');
  ...
  test.describe('Confirm Reservation Flow', () => {
    test('should open confirm modal when clicking confirm button', async ({ page }) => {
      ...
      const confirmButton = page.locator('button:has-text("确认"), button:has-text("确认预约")').first();
    ...
    test('should confirm order with payment required', async ({ page }) => {
      ...
      if (await confirmButton.count() > 0) {
        ...
        const modalConfirmBtn = page.locator('.ant-modal button:has-text("确定"), .ant-modal button:has-text("确认")').first();
  ...
  test.describe('Cancel Reservation Flow', () => {
    test('should open cancel modal when clicking cancel button', async ({ page }) => {
      ...
      // Find cancel button
      const cancelButton = page.locator('button:has-text("取消预约"), button.ant-btn-danger').first();
    ...
    test('should require cancel reason', async ({ page }) => {
      ...
      if (await cancelButton.count() > 0) {
        ...
        const modalConfirmBtn = page.locator('.ant-modal button:has-text("确定"), .ant-modal button:has-text("取消预约")').first();
        if (await modalConfirmBtn.count() > 0) {
          await modalConfirmBtn.click();
          await page.waitForTimeout(500);
    ...
    test('should cancel order with valid reason', async ({ page }) => {
      ...
      if (await cancelButton.count() > 0) {
        ...
        // Click confirm
        const modalConfirmBtn = page.locator('.ant-modal button:has-text("确定"), .ant-modal button.ant-btn-primary').first();
        if (await modalConfirmBtn.count() > 0) {
          await modalConfirmBtn.click();
          await page.waitForTimeout(1000);
