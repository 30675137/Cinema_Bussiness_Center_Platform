...
test.describe('订单状态更新功能 - User Story 4', () => {
  ...
  test('应该在订单详情页显示状态操作按钮', async ({ page }) => {
    // 找到一个"已支付"状态的订单
    const paidOrderRow = page.locator('.ant-table-tbody tr', {
  ...
  test('应该能够标记发货（已支付 → 已发货）', async ({ page }) => {
    ...
    // 等待确认对话框（如果有）
    const confirmButton = page.locator('.ant-modal button:has-text("确定"), .ant-popconfirm button:has-text("确定")')
  ...
  test('应该能够标记完成（已发货 → 已完成）', async ({ page }) => {
    // 找到一个"已发货"状态的订单
    const shippedOrderRow = page.locator('.ant-table-tbody tr', {
  ...
  test('应该能够取消订单（待支付 → 已取消）', async ({ page }) => {
    // 找到一个"待支付"状态的订单
    const pendingOrderRow = page.locator('.ant-table-tbody tr', {
  ...
  test('应该阻止非法状态转换（已完成订单不能取消）', async ({ page }) => {
    // 找到一个"已完成"状态的订单
    const completedOrderRow = page.locator('.ant-table-tbody tr', {
  ...
  test('应该根据订单状态显示正确的操作按钮', async ({ page }) => {
    ...
    // 1. 待支付订单 - 只能取消
    const pendingRow = page.locator('.ant-table-tbody tr', {
      has: page.locator('.ant-tag:has-text("待支付")')
    }).first()
    ...
    // 2. 已支付订单 - 可以发货或取消
    const paidRow = page.locator('.ant-table-tbody tr', {
      has: page.locator('.ant-tag:has-text("已支付")')
    }).first()
  ...
  test('应该在取消订单时必须输入取消原因', async ({ page }) => {
    // 找到可取消的订单
    const cancelableRow = page.locator('.ant-table-tbody tr', {
      has: page.locator('.ant-tag:has-text("待支付")')
    }).first()
