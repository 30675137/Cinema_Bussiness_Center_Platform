...
test.describe('订单详情页 - User Story 3', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    // 获取第一个订单的链接并提取订单ID
    const firstOrderLink = page.locator('.ant-table-tbody tr').first().locator('td').nth(0).locator('a')
  ...
  test('应该显示商品列表', async ({ page }) => {
    ...
    const hasList = await page.locator('.ant-list, .order-items').isVisible().catch(() => false)
  ...
  test('应该提供返回列表按钮', async ({ page }) => {
    ...
    // 查找返回按钮
    const backButton = page.locator('button:has-text("返回列表"), button:has-text("返回"), a:has-text("返回")')
  ...
  test('应该在订单不存在时显示404错误', async ({ page }) => {
    ...
    // 验证显示错误信息
    await expect(
      page.locator('.ant-result-404, .ant-empty, body')
    ).toContainText(/不存在|未找到|找不到/)
  })
