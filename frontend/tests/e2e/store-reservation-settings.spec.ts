...
test.describe('Store Reservation Settings Page', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page
      .waitForSelector('[data-testid="store-reservation-settings-page"]', { timeout: 10000 })
      .catch(async () => {
  ...
  test('should save changes when submitting valid form', async ({ page }) => {
    ...
    if (count > 0) {
      ...
      const submitButton = page
        .locator('button:has-text("保存"), button:has-text("确定"), .ant-btn-primary')
        .last();

      if (
        (await enabledSwitch.count()) > 0 &&
        (await daysInput.count()) > 0 &&
    ...
    if (checkboxCount > 1) {
      ...
      if ((await batchButton.count()) > 0) {
        ...
        const daysInput = page
          .locator('.batch-reservation-settings-form input[type="number"]')
          .first();
