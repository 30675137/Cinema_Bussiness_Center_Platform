...
test.describe('SPU 批量删除功能', () => {
  ...
  test('应该成功批量删除 SPU 并在刷新后数据不再出现', async ({ page }) => {
    ...
    // 2. 选中前 3 个 SPU 的复选框
    const firstThreeCheckboxes = page.locator('table tbody tr td:first-child input[type="checkbox"]').first()
    await firstThreeCheckboxes.check()

    const secondCheckbox = page.locator('table tbody tr:nth-child(2) td:first-child input[type="checkbox"]')
    await secondCheckbox.check()

    const thirdCheckbox = page.locator('table tbody tr:nth-child(3) td:first-child input[type="checkbox"]')
    await thirdCheckbox.check()

    // 3. 记录被删除的 SPU 名称(用于后续验证)
    const firstSPUName = await page.locator('table tbody tr:nth-child(1) td:nth-child(2)').textContent()
    const secondSPUName = await page.locator('table tbody tr:nth-child(2) td:nth-child(2)').textContent()
    const thirdSPUName = await page.locator('table tbody tr:nth-child(3) td:nth-child(2)').textContent()
    ...
    // 12. 再次验证被删除的 SPU 不在列表中
    const allSPUNamesAfterReload = await page.locator('table tbody tr td:nth-child(2)').allTextContents()
  ...
  test('应该正确显示批量删除进度和结果', async ({ page }) => {
    // 1. 选中 2 个 SPU
    const firstCheckbox = page.locator('table tbody tr:nth-child(1) td:first-child input[type="checkbox"]')
  ...
  test('应该在没有选中任何 SPU 时禁用批量删除按钮', async ({ page }) => {
    // 验证批量删除按钮初始状态为禁用
    const batchDeleteButton = page.locator('button:has-text("批量删除")')
  ...
  test('应该在取消删除确认后不执行删除', async ({ page }) => {
    ...
    // 4. 点击取消按钮
    const cancelButton = page.locator('.ant-modal-confirm-btns button.ant-btn:not(.ant-btn-primary)')
