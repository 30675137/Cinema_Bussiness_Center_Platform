...
### P1-001: 项目环境初始化
- [X] [SETUP-001] [P1] [Infra] 创建功能分支 `feat/O006-miniapp-channel-order` 并切换 `hall-reserve-taro/`
- [X] [SETUP-002] [P1] [Infra] 验证 Taro 项目依赖安装 (`npm install` 成功,版本 Taro 4.1.9 + React 18.3.1)
- [X] [SETUP-003] [P1] [Infra] 启动 H5 开发服务器验证基础环境 (`npm run dev:h5` 成功运行)
- [X] [SETUP-004] [P1] [Infra] 配置 `.specify/active_spec.txt` 指向 `specs/O006-miniapp-channel-order/spec.md`
...
- [X] [TYPE-001] [P1] [Foundational] 创建 `hall-reserve-taro/src/types/channelProduct.ts`:
...
- [X] [TYPE-002] [P1] [Foundational] 扩展 `hall-reserve-taro/src/types/channelProduct.ts`:
...
- [X] [TYPE-003] [P1] [Foundational] 修改 `hall-reserve-taro/src/types/order.ts`:
...
### P2-002: 样式基础设施
- [X] [STYLE-001] [P1] [Foundational] 创建 `hall-reserve-taro/src/styles/variables.scss`:
  ...
  - 定义圆角/阴影等通用样式变量
- [X] [STYLE-002] [P1] [Foundational] 创建 `hall-reserve-taro/src/assets/images/placeholders/`:
...
### P2-003: 工具函数
- [X] [UTIL-001] [P1] [Foundational] 创建 `hall-reserve-taro/src/utils/priceCalculator.ts`:
...
### P2-004: API 服务层
- [X] [API-001] [P1] [Foundational] 创建 `hall-reserve-taro/src/services/channelProductService.ts`:
  ...
  - 添加错误处理和重试逻辑 (3 次重试)
  - 添加 `@spec O006-miniapp-channel-order` 标识
- [X] [API-002] [P1] [Foundational] 修改 `hall-reserve-taro/src/services/orderService.ts`:
...
- [X] [HOOK-001] [P1] [Foundational] 创建 `hall-reserve-taro/src/hooks/useChannelProducts.ts`:
  ...
  - 添加加载状态和错误处理
- [X] [HOOK-002] [P1] [Foundational] 创建 `hall-reserve-taro/src/hooks/useChannelProductDetail.ts`:
...
- [X] [HOOK-003] [P1] [Foundational] 创建 `hall-reserve-taro/src/hooks/useOrders.ts`:
...
### P2-006: Zustand 状态管理
- [X] [STORE-001] [P1] [Foundational] 创建 `hall-reserve-taro/src/stores/orderCartStore.ts`:
...
### P3-001: 商品列表页面
- [X] [US1-001] [P1] [US1] 创建 `hall-reserve-taro/src/pages/channel-product-menu/index.tsx`:
  ...
  - 实现图片加载失败显示默认占位图
  - 添加 `@spec O006-miniapp-channel-order` 标识
- [X] [US1-002] [P1] [US1] 创建 `hall-reserve-taro/src/pages/channel-product-menu/index.less`:
  - 引入 `@import '@/styles/variables.scss'`
  - 实现分类标签栏样式 (参考原型配色和布局)
  - 实现商品卡片样式 (使用 rpx 单位适配)
  - 实现推荐标签样式
  - 实现列表滚动容器样式
- [X] [US1-003] [P1] [US1] 创建 `hall-reserve-taro/src/pages/channel-product-menu/index.config.ts`:
  - 配置页面标题 "点餐菜单"
  - 配置导航栏样式
...
### P4-001: 商品详情页面
- [X] [US2-001] [P1] [US2] 创建 `hall-reserve-taro/src/pages/channel-product-detail/index.tsx`:
  ...
  - 实现 Toast 提示 "已添加到购物车"
  - 添加 `@spec O006-miniapp-channel-order` 标识
- [X] [US2-002] [P1] [US2] 创建 `hall-reserve-taro/src/components/SpecSelector/index.tsx`:
  - 实现规格选择器组件 (可复用)
  ...
  - 实现选中状态高亮显示
  - 实现价格调整显示 (+5 元, -3 元)
- [X] [US2-003] [P1] [US2] 创建 `hall-reserve-taro/src/pages/channel-product-detail/index.less`:
  - 引入 `@import '@/styles/variables.scss'`
  - 实现商品详情样式 (参考原型商品详情页布局)
  - 实现规格选择器样式 (选项卡片/高亮状态/价格标签)
  - 实现底部固定"加入购物车"按钮样式
- [X] [US2-004] [P1] [US2] 创建 `hall-reserve-taro/src/pages/channel-product-detail/index.config.ts`:
  - 配置页面标题 "商品详情"
...
### P5-001: 购物车抽屉组件
- [X] [US3-001] [P1] [US3] 创建 `hall-reserve-taro/src/components/CartDrawer/index.tsx`:
  ...
  - 实现"提交订单"按钮 (购物车为空时禁用)
  - 添加 `@spec O006-miniapp-channel-order` 标识
- [X] [US3-002] [P1] [US3] 创建 `hall-reserve-taro/src/components/CartDrawer/index.less`:
  - 引入 `@import '@/styles/variables.scss'`
  - 实现抽屉弹窗样式 (参考原型购物车抽屉)
  - 实现购物车项卡片样式
  - 实现数量修改按钮样式
...
### P5-002: 购物车入口与订单提交
- [X] [US3-003] [P1] [US3] 修改 `hall-reserve-taro/src/pages/channel-product-menu/index.tsx`:
  - 添加购物车入口按钮 (右上角固定位置)
  - 显示购物车数量角标 (`useCartStore().totalQuantity`)
  - 点击按钮弹出购物车抽屉 (调用 `CartDrawer` 组件)
- [X] [US3-004] [P1] [US3] 创建 `hall-reserve-taro/src/pages/order-cart/index.tsx`:
  ...
  - 实现防抖处理 (防止重复提交)
  - 添加 `@spec O006-miniapp-channel-order` 标识
- [X] [US3-005] [P1] [US3] 创建 `hall-reserve-taro/src/pages/order-cart/index.less`:
  - 引入 `@import '@/styles/variables.scss'`
  - 实现订单确认页样式
...
### P6-001: 订单列表页面
- [X] [US4-001] [P1] [US4] 创建 `hall-reserve-taro/src/pages/member/my-orders/index.tsx`:
  ...
  - 实现分页加载 (上拉加载更多)
  - 添加 `@spec O006-miniapp-channel-order` 标识
- [X] [US4-002] [P1] [US4] 创建 `hall-reserve-taro/src/pages/member/my-orders/index.less`:
  - 引入 `@import '@/styles/variables.scss'`
  - 实现订单卡片样式 (参考原型会员-订单列表)
  - 实现订单状态标签样式 (不同状态不同颜色)
  - 实现空状态提示样式 ("暂无订单")
- [X] [US4-003] [P1] [US4] 创建 `hall-reserve-taro/src/pages/member/my-orders/index.config.ts`:
  - 配置页面标题 "我的订单"
...
### P6-002: 订单详情与状态轮询
- [X] [US4-004] [P1] [US4] 创建 `hall-reserve-taro/src/pages/member/order-detail/index.tsx`:
  ...
  - 添加 `@spec O006-miniapp-channel-order` 标识
- [X] [US4-005] [P1] [US4] 创建 `hall-reserve-taro/src/pages/member/order-detail/index.less`:
  - 引入 `@import '@/styles/variables.scss'`
  - 实现订单详情样式
  - 实现商品列表样式
  - 实现"再来一单"按钮样式
...
### P7-001: 全局配置与路由
- [X] [POLISH-001] [P1] [Cross] 修改 `hall-reserve-taro/config/index.ts`:
  - 添加新页面路由配置 (channel-product-menu, channel-product-detail, order-cart, member/my-orders, member/order-detail)
  - 配置页面权限 (需要登录的页面)
- [X] [POLISH-002] [P1] [Cross] 修改 `hall-reserve-taro/project.config.json`:
...
- [X] [SC-001] 完整订单流程≤2分钟 (计时测试)
- [X] [SC-002] 商品列表首屏加载≤2秒 (20条商品含图片,Performance API 测量)
- [X] [SC-003] 商品详情页加载≤1秒 (Performance API 测量)
- [X] [SC-004] 价格计算准确率 100% (单元测试覆盖所有规格组合)
- [X] [SC-005] 订单提交成功率≥95% (压力测试 100 个订单)
- [X] [SC-006] 订单状态更新延迟≤5秒 (轮询间隔测试)
- [X] [SC-007] 取餐通知到达率≥95% (Mock 推送通知测试)
- [X] [SC-008] 高峰期 100 并发无性能下降 (负载测试)
- [X] [SC-009] 90%用户首次使用成功下单 (用户测试,无需客服)
