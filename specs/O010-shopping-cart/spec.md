...
2. **Given** 购物车抽屉已打开, **When** 查看抽屉内容, **Then** 顶部显示标题栏（"订单汇总" + 圆形关闭按钮），中间显示商品列表（每个商品包含：圆角商品图片 14x14 + 商品名称 + 商品选项（如有，在名称下方以灰色小字显示"少冰 · 半糖"格式）+ 橙色价格 + 数量控制器），底部固定区域显示小计、优惠金额、实付金额（大号橙色字体）和支付按钮
...
1. **Given** 用户已添加商品到购物车, **When** 关闭小程序后重新打开（未超过7天）, **Then** 购物车中的商品数据保留（商品列表、数量、总金额），浮动购物车按钮显示正确的角标和金额
2. **Given** 用户在购物车中调整了商品数量, **When** 切换到其他页面（如会员中心）后返回商品列表, **Then** 购物车数据保持最新状态，商品卡片上的数量控制器显示正确数量
3. **Given** 购物车数据已持久化且超过7天, **When** 用户打开小程序, **Then** 系统自动清空购物车，显示空购物车状态
4. **Given** 购物车数据已持久化（未超过7天）, **When** 用户打开小程序但存储的商品已下架或不存在, **Then** 系统自动从购物车中移除无效商品，仅保留有效商品
...
- **FR-004**: 系统必须在购物车总金额 > 0 且用户位于商品列表页时，在页面底部距离底部导航 96px 位置显示固定浮动购物车按钮（其他页面不显示）
...
- **FR-010**: 购物车抽屉商品列表中每个商品必须显示圆角商品图片（14x14）、商品名称、商品选项（如有，在名称下方以灰色小字显示，格式如"少冰 · 半糖"）、橙色价格、数量控制器（灰色圆角背景）
...
- **FR-014**: 系统必须使用 Taro.setStorageSync('cart', cart) 将购物车数据持久化到本地存储，存储键名为 'cart'，同时保存时间戳字段用于过期检查
- **FR-015**: 系统必须在应用启动时使用 Taro.getStorageSync('cart') 从本地存储恢复购物车数据，检查时间戳是否超过7天，超过7天则自动清空购物车，未超过则验证商品是否仍然有效
...
- **FR-023**: 点击购物车抽屉底部的"立即支付"按钮时，系统必须使用 Taro.navigateTo() 跳转到订单确认页面（/pages/order-confirm/index），并传递购物车数据
...
- **购物车 (Cart)**: 表示用户的购物车状态
  ...
  - 时间戳 (timestamp): 数字，购物车最后更新的时间戳（毫秒），用于7天过期检查
...
- 假设购物车数据持久化使用 'cart' 作为存储键名，数据结构为 `{ items: CartItem[], timestamp: number }`，timestamp 用于7天过期检查
...
- 假设购物车数据持久化有效期为7天，超过7天的购物车数据在应用启动时自动清空（或用户主动清空、订单提交成功后清空）
...
- **前端模块**:
  - ProductCard 组件（O009-miniapp-product-list 已实现于 `miniapp-ordering-taro`，需新增数量控制器集成）
...
- Q: 项目目录结构确认 - 规格文档中多处引用了 `hall-reserve-taro` 目录，但您提到正确的项目路径应该是 `miniapp-ordering-taro`。应使用哪个目录？ → A: 使用 `miniapp-ordering-taro` 作为所有实现文件的项目目录
- Q: 浮动购物车按钮的显示范围 - 规格中提到"页面底部显示固定浮动购物车按钮"，但未明确该按钮应在哪些页面显示。 → A: 仅在商品列表页显示（与 UI 原型一致，避免在其他页面产生干扰）
- Q: 购物车商品选项（规格）的显示方式 - CartItem 实体包含 selectedOptions 字段（如"冰量: 少冰"，"糖度: 半糖"），但未明确这些选项在购物车 UI 中如何显示。 → A: 在购物车抽屉的商品名称下方以灰色小字显示选项（如"少冰 · 半糖"），便于用户区分同一商品的不同规格
- Q: "去结账"按钮的跳转行为 - 规格中提到点击"立即支付"按钮，但未明确点击后的跳转目标页面或行为。 → A: 跳转到订单确认页（显示商品明细、收货信息、支付方式选择），这是标准电商流程，允许用户在支付前最后确认订单信息
- Q: 购物车数据持久化的有效期 - 规格中假设"购物车数据持久化时间无限制"，但未明确在什么情况下应该自动清空购物车。 → A: 设置7天过期时间，购物车数据保存时同时记录时间戳，应用启动时检查超过7天的购物车数据自动清空
