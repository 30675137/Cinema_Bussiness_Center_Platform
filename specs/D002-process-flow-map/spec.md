...
作为影院业务中台的用户，我希望能够通过按钮快速切换业务全景中台服务地图（泳道架构）和业务流程地图（流程顺序），以便根据不同使用场景选择最合适的视图。

**Why this priority**: 两种视图提供不同维度的信息，切换功能是连接 D001 和 D002 功能的关键桥梁，必须在早期实现以验证整体设计的可行性。

**Independent Test**: 用户在 Dashboard 页面顶部看到"视图切换"按钮，点击后可以在"业务全景地图"和"业务流程地图"之间无缝切换，切换时页面状态保持（如折叠状态、滚动位置）。

**Acceptance Scenarios**:

1. **Given** 用户正在查看业务全景中台服务地图（泳道视图），**When** 用户点击"切换到流程视图"按钮，**Then** 页面平滑过渡到业务流程地图，显示流程顺序视图
2. **Given** 用户正在查看业务流程地图，**When** 用户点击"切换到泳道视图"按钮，**Then** 页面返回业务全景中台服务地图，显示泳道架构
...
- **FR-002**: 系统必须在 Dashboard 页面顶部提供视图切换按钮，允许用户在"业务全景地图"和"业务流程地图"之间切换
- **FR-003**: 系统必须在业务流程地图中展示流程箭头或连接线，清晰指示业务执行的先后顺序
...
- **业务流程步骤 (Process Step)**: 代表业务流程中的一个执行阶段，包含多个相关业务模块，按照执行顺序排列（如：阶段1-基础配置、阶段2-主数据准备、阶段3-业务执行、阶段4-分析优化）
...
- 假设视图切换按钮放置在页面顶部 Header 区域，与标题在同一行
- 假设业务流程分为 4 个主要阶段：基础配置 → 主数据准备 → 业务执行 → 分析优化
