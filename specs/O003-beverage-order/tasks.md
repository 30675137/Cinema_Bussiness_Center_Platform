...
- **B端 Frontend**: `frontend/src/features/beverage-order-management/`
...
## Phase 3: User Story 1 - C端饮品下单 (Priority: P1) 🎯 MVP

**Goal**: 顾客可以浏览饮品菜单、选择规格、提交订单、Mock支付、获得订单号和取餐号

**Independent Test**: 顾客打开小程序 → 浏览饮品菜单 → 选择饮品和规格 → 提交订单 → 支付 → 获得订单号和取餐号
...
## Phase 4: User Story 2 - B端订单接收与出品 (Priority: P1) 🎯 MVP
...
## Phase 5: User Story 3 - 订单历史与统计 (Priority: P2)

**Goal**: 顾客查看历史订单、复购；商家查看营业统计、导出报表

**Independent Test**: C端查看历史订单列表 → 筛选和查询 → 一键复购；B端查看统计报表和导出数据
...
- **User Story 1 (P1 MVP)**: Can start after Foundational (Phase 2) - No dependencies on other stories
  - Backend beverage APIs → C端菜单页 → C端订单流程
  - T037-T048 (Beverage APIs) MUST complete before T072-T073 (C端菜单/详情页)
  - T049-T063 (Order APIs) MUST complete before T074-T078 (C端订单流程)

- **User Story 2 (P1 MVP)**: Can start after Foundational (Phase 2) - Independent but integrates with US1 data
  - BOM integration → Order status management → B端 UI
  - T082-T090 (BOM) MUST complete before T094-T095 (status transitions)
  - T091-T102 (Backend) MUST complete before T103-T117 (B端 UI)
...
- **User Story 3 (P2)**: Can start after Foundational (Phase 2) - Uses historical data from US1/US2
  - Backend statistics APIs → C端/B端 UI
...
- **User Story 1**:
  ...
  - Backend APIs complete before C端 pages that consume them
...
- **User Story 2**:
  ...
  - Backend APIs complete before B端 pages
...
# Then sequentially: repositories → services → endpoints
...
3. Complete Phase 3: User Story 1 (C端下单流程)
4. **STOP and VALIDATE**: Test C端下单 independently (browse → select → order → pay)
5. Complete Phase 4: User Story 2 (B端出品流程)
6. **STOP and VALIDATE**: Test B端出品 independently (receive → produce → call)
7. **INTEGRATION TEST**: Test complete flow (C端下单 → B端出品 → C端取餐)
8. Deploy/demo MVP

### Incremental Delivery

1. Complete Setup + Foundational → Foundation ready
2. Add User Story 1 → Test independently → Deploy/Demo (C端可用!)
3. Add User Story 2 → Test independently → Deploy/Demo (B端可用, 完整MVP!)
4. Add User Story 3 → Test independently → Deploy/Demo (统计功能上线)
...
2. Once T020 (database migration) succeeds:
   - **Developer A**: User Story 1 (T037-T081) - C端下单
   - **Developer B**: User Story 2 (T082-T117) - B端出品
   - **Developer C**: User Story 3 (T118-T136) - 统计分析
...
- TanStack Query polling interval: 8 seconds for both B端 and C端
