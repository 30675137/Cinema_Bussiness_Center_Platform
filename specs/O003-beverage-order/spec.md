...
**Independent Test**: 可以独立测试 - B端工作人员打开管理后台(MVP阶段无需登录) → 实时接收新订单通知 → 查看订单详情 → 标记制作中 → 标记已完成 → 叫号通知顾客。可以用 mock 订单数据独立测试出品流程，不依赖 C端下单功能。

**Acceptance Scenarios**:

1. **Given** B端工作人员打开管理后台（Web/平板,MVP阶段无需登录），**When** 有新订单生成，**Then** 系统发出语音/震动提醒，订单列表自动刷新显示新订单（订单号、取餐号、饮品、规格、数量、下单时间）
...
### User Story 3 - B端饮品配置管理 (Priority: P1) 🎯 MVP

作为商品管理员，我希望在B端管理后台配置饮品菜单（新增/编辑/删除饮品、配置规格、维护配方），以便C端顾客能够浏览到最新的饮品菜单并完成下单。

**Why this priority**: 这是饮品订单系统的基础数据管理功能，必须先配置饮品和配方，C端才能展示菜单和下单。与US1/US2同等重要，是系统运作的前提。如果没有饮品配置功能，菜单将是空的，顾客无法下单。

**Independent Test**: 可以独立测试 - 商品管理员登录B端后台 → 新增饮品（填写名称/分类/价格/上传图片） → 配置规格（大小/温度/甜度/配料及价格调整） → 维护配方（关联原料SKU、设置用量） → 上架饮品 → C端验证菜单显示。可以用Mock原料库存数据独立测试配方配置，不依赖实际下单功能。

**Acceptance Scenarios**:

1. **Given** 商品管理员打开B端饮品管理页，**When** 查看饮品列表，**Then** 看到所有饮品按分类展示（分页显示，支持搜索和筛选），显示饮品名称、分类、价格、状态（上架/下架/缺货）、创建时间
2. **Given** 商品管理员在饮品列表页，**When** 点击"新增饮品"按钮，**Then** 打开饮品表单，填写名称/分类/基础价格/描述/上传主图和详情图/设置推荐标签/选择初始状态，提交后饮品创建成功
3. **Given** 商品管理员创建饮品后，**When** 进入规格配置页，**Then** 可以为该饮品添加多种规格（SIZE: 中杯+0元/大杯+3元；TEMPERATURE: 热/冰；SWEETNESS: 无糖/半糖/全糖；TOPPING: 珍珠+2元/椰果+2元），每个规格可设置价格调整和默认选项
4. **Given** 商品管理员配置规格后，**When** 进入配方（BOM）管理页，**Then** 可以为该饮品添加配方，关联原料SKU（从P001 SKU主数据中选择），设置每种原料的用量和单位，保存后配方生效用于BOM扣料
5. **Given** 商品管理员完成饮品/规格/配方配置，**When** 将饮品状态切换为"上架"，**Then** 饮品立即在C端菜单中显示，顾客可以浏览和下单
6. **Given** 商品管理员在饮品列表页，**When** 点击某个饮品的"编辑"按钮，**Then** 可以修改饮品信息（名称/价格/描述/图片/状态），修改后立即生效
7. **Given** 商品管理员在饮品列表页，**When** 点击某个饮品的"删除"按钮并确认，**Then** 饮品软删除（状态改为INACTIVE），不再在C端显示，但历史订单中仍保留该饮品信息
8. **Given** 商品管理员在饮品详情页，**When** 修改规格（如：大杯价格从+3元改为+5元），**Then** 修改立即生效，新订单使用新价格，已下单的历史订单不受影响

---

### User Story 4 - 订单历史与统计 (Priority: P2)
...
**B端饮品配置模块（US3）**:
- **FR-028**: 商品管理员必须能够查看饮品列表，支持分页显示（每页20条）、按名称搜索、按分类/状态筛选
- **FR-029**: 商品管理员必须能够新增饮品，填写名称/分类（COFFEE/TEA/JUICE/SMOOTHIE/MILK_TEA/OTHER）/基础价格/描述/上传主图和详情图（Supabase Storage）/设置推荐标签/选择初始状态（上架/下架）
- **FR-030**: 商品管理员必须能够编辑饮品信息（名称/价格/描述/图片/状态），修改立即生效，新订单使用新数据
- **FR-031**: 商品管理员必须能够删除饮品，软删除（状态改为INACTIVE），不再在C端显示，历史订单保留饮品快照
- **FR-032**: 商品管理员必须能够为饮品配置规格，支持4种规格类型（SIZE/TEMPERATURE/SWEETNESS/TOPPING），每个规格可设置名称/价格调整（正数加价/负数减价）/默认选中/排序权重
- **FR-033**: 商品管理员必须能够编辑/删除饮品规格，修改立即生效，新订单使用新规格，历史订单保留规格快照
- **FR-034**: 商品管理员必须能够切换饮品状态（上架ACTIVE/下架INACTIVE/缺货OUT_OF_STOCK），状态变更立即在C端生效
- **FR-035**: 商品管理员必须能够为饮品配置配方（BOM），关联原料SKU（从P001 SKU主数据中选择），设置每种原料的用量和单位（g/ml/个），配方可关联特定规格组合（如：大杯+珍珠）
- **FR-036**: 商品管理员必须能够编辑/删除配方，系统在删除配方前校验是否有使用该配方的未完成订单，如有则阻止删除并提示
- **FR-037**: 系统必须在配方配置时校验原料SKU存在性和单位一致性（配方用量单位必须与SKU主数据单位一致），校验失败则阻止保存并提示错误
...
- Q: B端工作人员认证方式? → A: 当前阶段B端的工作人员不用登录,用mock实现
- Q: 饮品配置功能是否包含配方（BOM）管理？ → A: 包含完整的配方管理（新增/编辑/删除配方、关联原料、设置用量）
- Q: 购物车入口位置？ → A: 菜单页右上角固定按钮（不随页面滚动移动，始终可见）
- Q: 购物车数量角标显示？ → A: 显示商品总数量（例如：2杯拿铁+1杯美式 = 角标显示"3"）
- Q: 购物车为空时的交互行为？ → A: 购物车为空时禁用按钮（图标变灰，点击无反应）
- Q: 点击"加入购物车"后的页面跳转行为？ → A: 停留在详情页，显示 Toast 提示"已添加到购物车"（用户可以继续调整数量或返回）
- Q: 购物车中相同饮品不同规格的处理方式？ → A: 分别作为独立商品项（例如：大杯热拿铁半糖加珍珠 x2、小杯冰拿铁全糖 x1 = 显示为2个独立条目）
...
- **用户认证**:
  - C端: 假设用户已通过静默登录（silent login）完成微信授权，所有订单相关操作需要用户登录状态
  - B端: MVP阶段工作人员无需登录,直接访问管理后台(使用Mock认证),假设B端在内网或店内受信任网络环境访问;Phase 2版本将实现完整的工作人员账号体系和权限管理
