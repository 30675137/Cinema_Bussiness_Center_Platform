...
- Q: 在 init 命令执行时,系统应该如何获取飞书多维表格(Base App)的地址? → A: 执行 `lark-pm init` 时交互式提示用户输入 Base App URL 或 Token
- Q: 当用户提供的 Base App Token/URL 无效或无权限访问时,系统应该如何处理? → A: 立即验证 Token/URL 有效性,如果无效则提示错误并重新询问用户输入(最多3次重试)
- Q: 如果用户提供的 Base App 中已经存在同名数据表(如已有"Tasks"表),系统应该如何处理? → A: 提示用户选择操作: 1)跳过已存在的表 2)覆盖已存在的表(会清空数据) 3)终止 init
- Q: 当多个团队成员(项目管理者/开发人员/测试人员)同时使用 `lark-pm` 管理项目时,不同角色对数据的访问和修改权限应该如何控制? → A: 完全依赖飞书 Base App 的原生权限管理,CLI 工具不实现权限系统。管理员在飞书 Base 中设置表级别或字段级别权限,CLI 执行操作时捕获飞书 API 返回的 403 错误并提示用户联系管理员
- Q: 当任务/Bug/测试记录超过1000条时,为了保证查询性能和用户体验,系统应该如何处理大数据量查询? → A: 所有 `list` 命令强制启用分页,默认每页显示20条记录。提供 `--page <页码>` 和 `--page-size <每页条数>` 参数(默认20,最大100)。显示分页信息和导航提示。用户可通过 `export` 命令导出全量数据
...
- **Base App Token/URL 验证失败**: 在 `lark-pm init` 交互式输入阶段，系统立即验证用户提供的 Base App Token/URL 有效性。如果验证失败（无效 Token、无权限访问、网络错误等），显示具体错误信息并重新提示用户输入，最多允许3次重试。3次失败后退出 init 流程并提示用户检查配置或网络连接。
- **Base App 中已存在同名数据表**: 在创建数据表前，系统检查 Base App 中是否已存在同名表（Tasks/TechnicalDebt/Bugs/Features/TestRecords）。如果存在同名表，提示用户选择操作：1) 跳过已存在的表，仅创建缺失的表；2) 覆盖已存在的表（警告：会清空该表的所有数据）；3) 终止 init 流程。用户选择后继续执行相应操作。
- **大量数据查询**: 所有 `list` 命令（如 `task-list`, `bug-list`, `debt-list`）强制启用分页，默认每页显示 20 条记录。用户可通过 `--page <页码>` 参数切换页面，通过 `--page-size <条数>` 参数调整每页显示条数（最大 100 条）。CLI 显示分页信息（如"显示第 1-20 条，共 156 条记录 | 第 1/8 页"）和导航提示（"使用 --page 2 查看下一页"）。需要查看全量数据时，建议使用 `export` 命令导出 Excel/CSV 文件。
- **权限控制**: CLI 工具不实现权限系统，完全依赖飞书 Base App 的原生权限管理。管理员在飞书 Base 中为不同角色设置表级别或字段级别权限（如项目管理者可修改 Epic/Sprint，开发人员仅可更新 Task 状态）。当 CLI 执行操作时，如果用户无权限，捕获飞书 API 返回的 403 错误并显示友好提示："权限不足，请联系管理员在飞书 Base 中授予相应权限"。
...
- **FR-016**: 系统必须在单个飞书 Base App 中组织数据，包含 5 个独立数据表：Tasks（任务）、TechnicalDebt（技术债）、Bugs（缺陷）、Features（功能矩阵）、TestRecords（测试记录）。用户在执行 `lark-pm init` 时通过交互式提示输入已存在的 Base App URL 或 Token，系统基于该 Base App 创建所需的数据表结构，而非自动创建新的 Base App
- **FR-017**: 系统必须支持导出数据为 Excel 或 CSV 格式，用于离线分析和报表
- **FR-018**: 系统必须在飞书 API 调用失败时采用指数退避重试策略（最多3次，间隔1s/2s/4s），失败后记录日志并提示用户
- **FR-019**: 系统不实现独立的权限系统，完全依赖飞书 Base App 的原生权限管理。当 CLI 执行操作遇到 403 权限错误时，捕获异常并显示友好提示："权限不足，请联系管理员在飞书 Base 中授予相应权限"
- **FR-020**: 所有 `list` 命令（task-list/bug-list/debt-list/feature-list/test-list）必须强制启用分页，默认每页显示 20 条记录。必须提供 `--page <页码>` 参数（从 1 开始）和 `--page-size <条数>` 参数（默认 20，最大 100）。必须在输出中显示分页信息（"显示第 X-Y 条，共 Z 条记录 | 第 M/N 页"）和导航提示（"使用 --page <页码> 查看其他页面"）
