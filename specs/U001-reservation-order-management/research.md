...
## 9. Profile 页面集成研究 (User Story 7)

### 9.1 F001-miniapp-tab-bar 依赖分析
...
**当前 Profile 页面状态**: 占位页面,仅包含标题和"个人中心开发中..."提示文本
...
**推荐 UI 组件**:
- 使用 `@tarojs/components` 原生组件: `View`, `Text`, `Image`
- 角标显示: 自定义 CSS 实现或使用 Taro UI Badge 组件
- 菜单项: 自定义 Cell 组件(图标 + 文字 + 箭头 + 可选角标)
...
**待处理订单定义**:
- 状态为 `PENDING`(待确认)
- 或状态为 `CONFIRMED` 且 `requiresPayment = true`(已确认待支付)
...
| 决策点 | 选择 | 理由 |
|--------|------|------|
| UI 组件 | 原生 @tarojs/components + 自定义样式 | 保持跨端兼容性,减少依赖 |
| 角标实现 | 自定义 CSS | 简单场景无需引入额外组件库 |
| 登录检测 | Taro.getStorageSync | Taro 标准存储 API |
| 订单计数 | 新增 API 端点 | 避免拉取完整列表,提升性能 |
