...
- Q: 右侧面板的默认状态与行为 → A: 初始关闭，点击新建或行操作时打开
- Q: 筛选条件的保存方式 → A: 仅当前会话保存，页面刷新后重置
- Q: 表格的默认排序方式 → A: 按更新时间降序，最新修改的在前
- Q: 右侧面板的关闭时机 → B: 明确关闭按钮，未保存时确认提醒
- Q: 批量操作的进度反馈方式 → A: Toast消息显示成功/失败数量
- Q: Mock数据的管理策略 → A: 使用localStorage进行数据持久化，支持页面刷新后数据保持，模拟完整的CRUD操作
- Q: 权限验证的Mock实现方式 → A: 完全跳过权限验证，所有功能默认可用，专注UI展示
- Q: Mock数据的初始化策略 → A: 预置50-100个模拟商品数据，覆盖所有状态和类目，包含真实感的名称、价格和描述
- Q: 文件上传功能的Mock处理 → A: 仅支持文件选择和预览，不上传不存储，关闭面板后清空
- Q: 导出功能的Mock实现 → A: 生成CSV格式文本，触发浏览器下载，但数据为模拟内容
...
- 批量导出（导出勾选项或当前筛选结果）
- 批量状态变更（草稿→审核，审核→草稿等）
- 操作反馈：Toast消息显示成功/失败数量
- 批量操作预期耗时：≤15秒

### 3.5 右侧面板功能

**FR-11: 面板模式与状态管理**
- **默认状态**：面板初始关闭，点击「+新建商品」或表格行操作时打开
- **创建模式**：新建商品，默认草稿状态
- **编辑模式**：编辑草稿或驳回状态商品
- **查看模式**：只读查看待审核商品，已发布商品需通过草稿版本编辑
- **已发布编辑模式**：点击编辑会创建新草稿版本，审核后替换原商品
- **关闭机制**：明确的关闭按钮，未保存时显示确认提醒对话框
...
### 7.4 前端Mock实现约束
- **数据持久化**：使用localStorage进行数据持久化，支持页面刷新后数据保持
- **权限验证**：完全跳过权限验证，所有功能默认可用，专注UI展示
- **API模拟**：使用setTimeout模拟网络延迟，提供真实的异步体验
- **数据初始化**：预置50-100个模拟商品数据，覆盖所有状态和类目，包含真实感的名称、价格和描述
- **状态一致性**：确保前端状态管理与localStorage数据保持同步
- **文件上传处理**：仅支持文件选择和预览，不上传不存储，关闭面板后清空
- **导出功能处理**：生成CSV格式文本，触发浏览器下载，但数据为模拟内容
