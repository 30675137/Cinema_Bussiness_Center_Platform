...
**Feature Branch**: `P006-fix-sku-edit-data`
**Created**: 2025-12-31
**Status**: Draft
...
- Q: 数据加载失败时的可观测性要求 - SPU/BOM加载失败时应捕获哪些日志信息？ → A: 记录到前端日志服务和后端应用日志,包含SKU ID、失败类型、错误详情
- Q: SKU编辑页面的访问权限控制 - 是否需要基于角色的访问控制？ → A: 所有运营人员均可访问SKU编辑页面（无角色区分）
- Q: BOM配方大量数据的性能优化策略 - 多少原料以上需要优化？ → A: 超过10种原料使用虚拟滚动
...
- 当BOM配方包含大量原料（超过10种），页面性能是否受影响？系统应使用虚拟滚动技术渲染BOM配方列表，确保即使配方包含数十种原料也能流畅滚动和快速加载。
...
- **NFR-001**: 系统MUST在数据加载失败时记录完整日志信息到前端日志服务和后端应用日志，日志MUST包含：SKU ID、失败类型（SPU加载失败/BOM加载失败）、HTTP状态码、错误消息、时间戳、用户ID，以便快速定位和排查问题
- **NFR-002**: SKU编辑页面的访问权限控制范围限定为：所有已认证的运营人员均可访问，无需基于角色的细粒度权限控制（简化此bugfix的实现范围）
- **NFR-003**: 当BOM配方包含超过10种原料时，系统MUST使用虚拟滚动技术渲染BOM配方列表，确保页面滚动流畅性（60 FPS）和快速加载（首屏渲染时间不超过1.5秒）
