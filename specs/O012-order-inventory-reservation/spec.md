...
- **FR-008**: 系统必须在BOM展开时应用损耗率配置，按公式（标准用量 × (1 + 损耗率)）计算实际预占数量，损耗率默认为0%，支持在BOM配方中配置
...
- **InventoryReservation（库存预占记录）**: 记录订单预占库存的明细，包含订单ID、门店ID、SKU ID、预占数量、预占时间、预占状态（ACTIVE/RELEASED/EXPIRED）、创建人、释放时间
