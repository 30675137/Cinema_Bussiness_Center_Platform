...
│   ├── TransactionList.tsx          # 流水列表（支持时间筛选、行点击、详情按钮）
│   ├── TransactionDetailDrawer.tsx  # 流水详情抽屉（显示完整流水信息）
│   ├── TransactionQuantityTag.tsx   # 流水数量标签（入库绿色+/出库红色-）
│   ├── ApprovalList.tsx             # 审批列表
│   ├── AdjustmentStatusTag.tsx      # 调整状态标签
│   ├── SafetyStockEditor.tsx        # 安全库存编辑器（支持乐观锁冲突处理）
│   └── AdjustmentDetailDrawer.tsx   # 调整详情抽屉
├── hooks/
│   ├── useInventoryAdjustment.ts    # 调整相关Hook
│   ├── useAdjustmentReasons.ts      # 调整原因Hook
│   ├── useApproval.ts               # 审批Hook
│   ├── useSafetyStock.ts            # 安全库存Hook（含冲突处理）
│   └── useTransactions.ts           # 流水查询Hook
├── services/
...
| 更新安全库存 | PUT | /api/inventory/{id}/safety-stock | 更新安全库存（支持乐观锁） |
...
classDiagram
    ...
    InventoryDetailDrawer --> TransactionDetailDrawer
...
sequenceDiagram
    ...
    U->>F: 打开库存详情抽屉
    U->>F: 切换到"流水记录"标签页
    F->>A: GET /api/transactions?skuId=xxx&storeId=yyy
    A->>D: 查询库存流水记录
    D-->>A: 返回流水列表
    A-->>F: 返回分页流水数据
    F->>U: 显示TransactionList组件
    
    alt 点击流水行
        U->>F: 点击某行流水记录
        F->>F: 打开TransactionDetailDrawer
        F->>U: 显示流水详情(交易概览/库存变化/来源信息/操作信息)
    else 点击详情按钮
        U->>F: 点击"详情"按钮
        F->>F: 打开TransactionDetailDrawer
        F->>U: 显示流水详情
...
当安全库存更新成功后，系统会自动创建一条流水记录：

```java
// 后端实现 - SafetyStockController.createSafetyStockTransaction()
private void createSafetyStockTransaction(StoreInventory inventory, 
    BigDecimal oldSafetyStock, BigDecimal newSafetyStock) {
    
    InventoryTransaction txn = new InventoryTransaction();
    txn.skuId = inventory.getSkuId();
    txn.storeId = inventory.getStoreId();
    txn.transactionType = "safety_stock_update"; // 安全库存更新类型
    txn.quantity = 0; // 不影响实际库存数量
    txn.stockBefore = inventory.getOnHandQty().intValue();
    txn.stockAfter = inventory.getOnHandQty().intValue();
    txn.availableBefore = inventory.getAvailableQty().intValue();
    txn.availableAfter = inventory.getAvailableQty().intValue();
    ...
    txn.remarks = String.format("安全库存从 %s 调整为 %s", 
            oldSafetyStock.intValue(), newSafetyStock.intValue());
...
| 字段 | 值 | 说明 |
|------|------|------|
| transactionType | `safety_stock_update` | 安全库存更新类型 |
| quantity | `0` | 安全库存变更不影响实际库存 |
| sourceType | `safety_stock_config` | 来源类型：安全库存配置 |
| sourceDocument | `SAFETY-XXXXXXXX` | 库存记录ID前8位 |
| remarks | "安全库存从 X 调整为 Y" | 变更说明 |
...
| V1.1 | 2025-12-26 | 新增流水详情抽屉组件、安全库存更新流水记录、流水查看时序图 | 系统自动生成 |
