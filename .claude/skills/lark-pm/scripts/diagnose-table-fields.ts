...
async function main() {
  ...
  // 跳过 Lark 客户端验证，直接使用配置
  const client = { searchRecords: async () => ({ items: [], total: 0 }) } as any

  try {
    const realClient = new LarkClient()
    Object.assign(client, realClient)
  } catch (error) {
    console.log(chalk.yellow('⚠️  无法初始化 Lark 客户端，尝试使用备用方案'))
  }
