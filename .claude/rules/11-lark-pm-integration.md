...
| Spec表 Table ID | `tblpQ6i8HRR0IInZ` |
...
| 字段名 | 类型 | 说明 |
|--------|------|------|
| Spec标识 | Text | specId (如 `O008`) |
| 状态 | SingleSelect | `To Do` / `Doing` / `Done` |
| 平台 | MultiSelect | 所属平台 |
| Spec说明 | Text | 功能说明 |
| 相关文档 | Text | 关联的文档链接（纯文本格式） |
| 测试报告 | Text | 测试报告链接 |
...
通过 Lark MCP 上传文档后，**必须**自动授予指定人员**完全管理权限(full_access)**，包含移动权限。

**默认授权人员**：

| 邮箱 | 权限 | 说明 |
|------|------|------|
| `30675137@qq.com` | **full_access** | 项目负责人，所有文档可编辑、可移动 |
...
await mcp__lark-mcp__drive_v1_permissionMember_create({
  data: {
    ...
    member_type: "openid",
    perm: "full_access"  // 必须使用 full_access，不能使用 edit
  },
...
| `view` | 只读 | 外部人员查看 |
| `edit` | 可编辑 | ❌ 不推荐，缺少移动权限 |
| `full_access` | **完全管理** | ✅ **默认使用**，包含编辑+移动权限 |

**重要**：所有通过 Lark PM 创建的文档**必须**使用 `full_access` 权限，确保用户可以移动文档到其他文件夹。

### R11.8 文档绑定到 Spec 表

上传文档后，应绑定到对应 Spec 记录的"相关文档"字段：

```typescript
// 1. 查询 Spec 记录
const spec = await mcp__lark-mcp__bitable_v1_appTableRecord_search({
  path: {
    app_token: "Y05Mb7greapFiSseRpoc5XkXnrb",
    table_id: "tblpQ6i8HRR0IInZ"  // Spec 表
  },
  data: {
    filter: {
      conjunction: "and",
      conditions: [
        { field_name: "Spec标识", operator: "contains", value: ["O008"] }
      ]
    }
  }
});

// 2. 更新"相关文档"字段（使用纯文本格式）
await mcp__lark-mcp__bitable_v1_appTableRecord_update({
  path: {
    app_token: "Y05Mb7greapFiSseRpoc5XkXnrb",
    table_id: "tblpQ6i8HRR0IInZ",
    record_id: spec.data.items[0].record_id
  },
  data: {
    fields: {
      "相关文档": "文档名称: https://xxx.feishu.cn/docx/xxx"  // 纯文本格式
    }
  }
});
```

**注意**："相关文档"字段使用纯文本格式，不要使用 URL 数组格式。
...
// ✅ 正确
{ "执行结果": "## 已完成\n- Task 1\n- Task 2" }
{ "相关文档": "O008-API文档: https://xxx.feishu.cn/docx/xxx" }

// ❌ 错误
{ "执行结果": [{"text": "已完成", "type": "text"}] }
{ "相关文档": [{"link": "https://...", "text": "文档", "type": "url"}] }
```
...
```typescript
// ✅ 正确
{ "file_name": "O008-API接口文档" }  // 11个字符

// ❌ 错误
{ "file_name": "O008-channel-product-category-migration-API接口文档" }  // 超过27字符
```

### 错误 5：用户访问令牌过期

**原因**：user_access_token 已过期

**解决**：使用 tenant_access_token（设置 `useUAT: false`）
...
- ❌ **禁止上传文档后使用 `edit` 权限（必须使用 `full_access`）**
- ❌ 禁止上传文档后不绑定到 Spec 表的"相关文档"字段

## 相关文档

- **Skill 文档**: `~/.claude/skills/lark-project-management/skill.md`
- **项目规则**: `.claude/rules/00-project-overview.md`

---

**版本**: 1.0.0 | **创建日期**: 2026-01-04
